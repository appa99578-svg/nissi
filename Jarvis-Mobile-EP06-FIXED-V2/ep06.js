...
(() => {
  ...
  function weatherLocation(text) {
    return text
      .replace(/\b(weather|wether|temperature|temp|report|reportu)\b/gi, " ")
      .replace(/\b(in|at|for|lo|loki|ki|nundi|nunchi|daggara)\b/gi, " ")
      .replace(/\b(cheppu|cheppava|chuddam|choodu|kavali|kavaali|please|ivvu|ivvava)\b/gi, " ")
      .replace(/\s+/g, " ").trim();
  }

  const WEATHER_ALIASES = {
    "tempalli": { latitude: 16.59014, longitude: 80.83019, label: "Tempalli, Andhra Pradesh" },
    "tempalli vijayawada": { latitude: 16.59014, longitude: 80.83019, label: "Tempalli, Andhra Pradesh" },
    "tempalle": { latitude: 16.59014, longitude: 80.83019, label: "Tempalli, Andhra Pradesh" }
  };

  async function weather(raw) {
    let place = weatherLocation(raw);
    const aliasKey = place.toLowerCase().replace(/\s+/g, " ").trim();
    if (WEATHER_ALIASES[aliasKey]) {
      const a = WEATHER_ALIASES[aliasKey];
      return weatherByCoords(a.latitude, a.longitude, a.label);
    }
  ...
  async function exchange(raw) {
    ...
    const found = [];
    for (const token of cleaned.match(/[a-z]+/g) || []) {
      if (aliases[token] && !found.includes(aliases[token])) found.push(aliases[token]);
    }
    const from = found[0], to = found[1];
    if (!from || !to || from === to) {
      throw new Error("Currency pair ardham kaaledu. Example: 100 USD to INR.");
    }
    ...
    try {
      ...
      const d = await r.json();
      if (typeof d?.rates?.[to] === "number") rate = d.rates[to];
    } catch (e) { lastError = e; }
    ...
    if (rate == null) {
      try {
        ...
        const d = await r.json();
        if (d?.result !== "success") throw new Error(d?.error || "ExchangeRate failed");
        if (typeof d?.rates?.[to] === "number") rate = d.rates[to];
      } catch (e) { lastError = e; }
    }

    if (rate == null) {
      throw new Error("Currency service unavailable. Please try again.");
    }
  ...
  function convert(raw) {
    ...
    if (!num) throw new Error("Conversion ardham kaaledu. Example: 10 km to miles.");
    const value = Number(num[0]);

    const words = [...cleaned.matchAll(/[a-z]+/gi)].map(m => m[0]);
    const units = words.filter(w => unitNames[w]);
    if (units.length < 2) {
      throw new Error("Conversion ardham kaaledu. Example: 10 km to miles.");
    }
    const from = unitNames[units[0]], to = unitNames[units[1]];
    return doConversion(value, from, to);
  }

  function doConversion(v, a, b) {
  ...
  function parseCommand(raw) {
    ...
    // Weather first: supports Telugu-English phrases and common misspelling "wether".
    if (/\b(weather|wether|temperature|temp)\b/i.test(t)) return ["weather", t];
    if (/(వాతావరణం|వెదర్|టెంపరేచర్)/i.test(t)) return ["weather", t];

    if (/\b(timer|set timer|timer pettu|timer petu|timer set|minutes? timer|seconds? timer|hours? timer|nimish[a-z]* timer)\b/i.test(t) ||
        /\d+(?:\.\d+)?\s*(?:min(?:ute)?s?|sec(?:ond)?s?|hours?|hrs?|hour|nimish[a-z]*)\b/i.test(t) ||
        /(టైమర్|నిమిష)/i.test(t)) return ["timer", t];
    ...
    const currencyWord = /\b(?:usd|inr|eur|gbp|jpy|aud|cad|sgd|aed|dollar|dollars|rupee|rupees|rs|euro|euros|pound|pounds|yen)\b/i;
    if (/\b(exchange|currency|convert currency|forex)\b/i.test(t) ||
        (currencyWord.test(t) && /\b(?:to|in|into|ki|loki|lo|nundi|nunchi)\b/i.test(t))) return ["exchange", t];

    if (/\b(news|latest news|breaking news|today.?s news)\b/i.test(t) || /(వార్తలు|న్యూస్)/i.test(t)) return ["news", t];
    if (/\b(web search|search google|google search|search for|search)\b/i.test(t)) return ["web", t];
    if (/\b(play|youtube|song|songs|music|paata|paatalu)\b/i.test(t)) return ["songs", t];
    if (/\b(maps?|navigate|navigation|directions|route|vellali|nunchi|nundi)\b/i.test(t) && /\b(to|ki|loki|nunchi|nundi)\b/i.test(t)) return ["maps", t];
    if (/\b(wiki|wikipedia)\b/i.test(t)) return ["wiki", t];
    if (/\b(random number|random)\b/i.test(t)) return ["random", t];
    if (/\b(convert|conversion|change|cheyyi|chey|convert cheyyi)\b/i.test(t) && /\d/.test(t)) return ["convert", t];

    // Unit phrases without the word "convert": "10 km to miles", "5 kg pounds lo".
    if (/\d+(?:\.\d+)?\s*(?:km|kilometers?|miles?|mi|meters?|m|kg|kilograms?|g|grams?|lb|pounds?|c|celsius|f|fahrenheit)\b/i.test(t) &&
        /\b(?:to|in|into|lo|loki|ki)\b/i.test(t)) return ["convert", t];
    return null;
  }
