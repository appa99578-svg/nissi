...
(() => {
  ...
  async function exchange(raw) {
    ...
    const codes = [...cleaned.matchAll(/\b(?:usd|inr|eur|gbp|jpy|aud|cad|sgd|aed)\b/gi)].map(x => x[0].toUpperCase());
  ...
  function convert(raw) {
    const words = raw.toLowerCase()
      .replace(/\b(convert|conversion|change|cheyyi|chey|cheppu|cheppandi|calculate)\b/g, " ")
      .replace(/\b(ni|nu|nundi|nunchi|lo|loki|ki|into|in|to|ga)\b/g, " ")
      .replace(/\s+/g, " ").trim();

    const aliases = {
      kilometers:"km", kilometer:"km", km:"km", kms:"km",
      ...
      celsius:"c", celcius:"c", c:"c", centigrade:"c",
      fahrenheit:"f", f:"f"
    };

    // Also accept Telugu-English forms such as: "10 km miles lo", "10 kilos pounds ki".
    const m = words.match(/(-?\d+(?:\.\d+)?)\s*([a-z]+)\s+(?:to|in|into)?\s*([a-z]+)/i);
    if (!m) {
      // More tolerant fallback: first number + first known unit + last known unit.
      const num = words.match(/-?\d+(?:\.\d+)?/);
      const found = [...words.matchAll(/\b[a-z]+\b/gi)].map(x => x[0]).filter(x => aliases[x]);
      if (!num || found.length < 2) throw new Error("Conversion ardham kaaledu. Example: 10 km to miles.");
      return doConversion(Number(num[0]), aliases[found[0]], aliases[found[1]]);
    }
    const v = Number(m[1]), a = aliases[m[2]], b = aliases[m[3]];
    if (!a || !b) throw new Error("Unknown unit. Try km, miles, kg, lb, C or F.");
    return doConversion(v, a, b);
  }
